...
- `tests/guide-renewal.test.mjs`, `tests/refresh-self-source.test.mjs` — plan-based renewal, the streaming guide filter, the one-request Publish build, 24/7 schedule generation and titles, version pruning, and the masked public health report.
- `tests/auto-refresh.test.mjs`, `tests/guide-freshness.test.mjs`, `tests/local-scheduler.test.mjs`, `tests/publish-onboarding.test.mjs` — scheduling, failure retries and freshness reporting.
- `tests/no-billed-bindings.test.mjs` — fails if `wrangler.toml` or `package.json` ever declares a usage-billed Cloudflare product (Durable Objects, Browser Run).
...
| `POST /api/build-from-plan` | `{ m3uText, plan, overrides?, baseXml? }` — `plan` is `{ [channelName]: link \| { channelId: null } }` for every channel | `{ ok, m3u, xml, counts }` — the final playlist and guide in one request, built by the same code as the scheduled refresh (guides streamed and filtered to your channels; no searching). This is what **Publish** and **Export** use. |
| `POST /api/publish` | `{ slug, m3uContent?, xmlContent?, overrides?, plan? }` (at least one content field) — `overrides` holds every manual link and `plan` every channel's match; see **Keeping a published playlist fresh automatically** | `{ ok, slug }` — then served at `/iptv/<slug>.m3u` and/or `/epg/<slug>.xml` |
...
- **24/7 channels get a generated schedule.** A channel detected as 24/7 (a film or show on a loop) that ends up with no real programmes gets 3-hour blocks titled after the show ("24/7 Seinfeld S07 [VIP]" → "Seinfeld Season 7"), running four days ahead with the channel's logo, and a TMDB title and description where one is matched. Titles are tidied: provider typos and odd spacing are corrected, and ALL-CAPS names get normal casing. A small curated table (`TWENTY_FOUR_SEVEN_FIXES` in `shared/core.js`) also gives film-series channels a proper title and description. For example, "24/7 Harry Poter" becomes **Harry Potter** ("The Harry Potter films on repeat") and "The Hobbit And Lord Of TheRings" becomes **The Lord of the Rings & The Hobbit**. Add a line there to fix another channel. Blocks are tagged `24/7` and regenerated on every renewal. They're excluded when judging a guide's freshness, so they can't hide a real schedule that's about to run out, and a real guide for the channel always wins.
