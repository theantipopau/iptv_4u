...
**Regenerate**: `scripts/trim_epgshare.mjs` is the generalized version of `trim_au_sports.mjs` — instead of an exhaustive id list, it takes regex patterns on the command line, so re-trimming (or trimming a new country) doesn't require editing a script. See the script's header comment for exact usage.

## South Africa (SuperSport) — no repo file needed

Unlike the guides above, `epgshare01`'s South Africa file (`epg_ripper_ZA1.xml.gz`, ~2.9MB decompressed) already fits comfortably under the 15MB cap as-is, so there's nothing to trim or commit — paste `https://epgshare01.online/epgshare01/epg_ripper_ZA1.xml.gz` directly into the "Your own EPG/guide URL(s)" field. It carries the full DStv SuperSport lineup (channels 200-215, 236) with real schedules — Blitz, Grandstand, PSL, Premier League, LaLiga, Football, Football Plus, Variety 2-4, Action, Rugby, Cricket, Golf, Tennis, Motorsport, WWE — plus Racing 240.

Confirmed against the live `iptv-org` catalog (used for identification/logos) that every one of those channels' exact DStv names already resolves at a perfect score, **except**:

- **Rugby, Cricket, Golf, Motorsport, Tennis, Grandstand**: this `epgshare01` source names these single-topic channels after the sport alone (`RUGBY.za`, `CRICKET.za`, ...), while `iptv-org`'s catalog lists them under SuperSport's full branding (`SuperSportRugby.za`, ...). Both still resolve — a query like "SuperSport Rugby" scores ~0.5 against the bare `RUGBY.za` entry via word-overlap, above the confidence threshold — just not at the same near-perfect confidence as an exact match, since the two sides only share one word out of two.
- **WWE (channel 236)**: `iptv-org` lists this one as `WWEChannelAfrica.za` ("WWE Channel Africa") — neither "SuperSport WWE" (the DStv guide name) nor "WWE Channel" (its on-screen name) shares more than one word with that, which scored just under the confidence threshold. Fixed directly via a known-alias entry in `shared/core.js` (`KNOWN_ALT_NAMES`, same mechanism as the AU Fox Sports/Kayo fix below) — both now resolve at a perfect score.
