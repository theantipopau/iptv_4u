...
Unlike the guides above, `epgshare01`'s South Africa file (`epg_ripper_ZA1.xml.gz`, ~2.9MB decompressed) already fits comfortably under the 15MB cap as-is, so there's nothing to trim or commit — paste `https://epgshare01.online/epgshare01/epg_ripper_ZA1.xml.gz` directly into the "Your own EPG/guide URL(s)" field. It carries the full SuperSport lineup with real schedules: Premier League, Football, LaLiga, Rugby, Cricket, Golf, Tennis, Motorsport, Grandstand, Variety 1-4, Action, Events, PSL, School, plus Racing 240.

**Naming note**: this source names the single-topic channels after the sport alone — `RUGBY.za`, `CRICKET.za`, `GOLF.za`, `MOTORSPORT.za`, `TENNIS.za`, `GRANDSTAND.za` — while `iptv-org`'s own catalog (used for identification/logos) lists the same channels under SuperSport's full branding — `SuperSportRugby.za`, `SuperSportCricket.za`, etc. Both resolve correctly today (a query like "SuperSport Rugby" scores ~0.5 against the bare `RUGBY.za` entry via word-overlap, above the confidence threshold) but expect a moderate rather than a perfect score on these specific channels, since the two sides only share one word out of two. If a match doesn't come through confidently enough, look for it in the search dialog's candidate list rather than assuming it's missing entirely.
